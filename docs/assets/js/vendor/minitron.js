var Constant;
(function (Constant) {
    Constant.TAO = 6.283185307179586;
})(Constant || (Constant = {}));
var Color;
(function (Color) {
    function hex2ARGBString(hex, opacity) {
    ...
    Color.hex2ARGBString = hex2ARGBString;
})(Color || (Color = {}));
var Minitron = (function () {
    function Minitron() {
        this.options = {
            background: '#000000',
            color: '#00FFFF',
    ...
    Minitron.prototype.renderpanel = function () {
        for (var x = Minitron.offset; x < this.width; x += Minitron.distance)
            for (var y = Minitron.offset; y < this.height; y += Minitron.distance)
                this.renderpixel(this.ctxpan, x, y, this.options.off);
    };
    Minitron.prototype.rendertext = function () {
        this.ctxtxt.clearRect(0, 0, this.width, this.height);
        var u = this.width - Minitron.distance * (this.frame), x = u, y = Minitron.offset;
        for (var i = 0; i < this.message.length; ++i) {
            var char = Minitron.CHARSET[this.message.charAt(i)] || Minitron.CHARSET['?'];
            for (var j = 0; j < char.length; ++j) {
                for (var k = 0; k < char[j].length && x < this.width; ++k, x += Minitron.distance)
                    if (char[j].charAt(k) === '.')
    ...
    Minitron.prototype.renderpixel = function (context, x, y, state) {
        context.beginPath();
    ...
    Minitron.prototype.setMessage = function (message) {
        this.message = message;
        return this;
    };
    Minitron.prototype.setOptions = function (options) {
        for (var key in options)
            this.options[key] = options[key];
        this.options.off = Color.hex2ARGBString(this.options.color, 0.2);
        return this;
    };
    Minitron.prototype.render = function () {
        this.renderpanel();
        this.rendertext();
    };
    Minitron.make = function (message, options) {
        var minitron = new Minitron();
    ...
    // panel
    Minitron.DEFAULT_HEIGHT = 40;
    Minitron.ROWS = 7;
    Minitron.COLS = 5;
    Minitron.PADDING = 2;
    // pixel
    Minitron.padding = 0.5;
    Minitron.radius = Minitron.DEFAULT_HEIGHT / (2 * (Minitron.ROWS + 2 * Minitron.PADDING)) - Minitron.padding;
    Minitron.offset = Minitron.padding + Minitron.radius;
    Minitron.distance = 2 * Minitron.offset;
    return Minitron;
}());
